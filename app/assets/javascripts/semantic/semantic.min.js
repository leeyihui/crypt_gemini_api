...
else if(isCheckbox){$field.prop('checked',!1)}
...
allValid=!0;if(field){module.verbose('Checking if field is valid',field);return module.validate.field(validation[field],field,!1)}
...
settings=$.extend(!0,{},$.fn.form.settings,parameters);validation=$.extend({},$.fn.form.settings.defaults,settings.fields);module.verbose('Extending settings',validation,settings)}}
...
if(isCheckbox){if(isChecked){values[name].push(value||!0)}
else{values[name].push(!1)}}
else{values[name].push(value)}}
else{if(isRadio){if(values[name]===undefined||values[name]==!1){values[name]=(isChecked)?value||!0:!1}}
else if(isCheckbox){if(isChecked){values[name]=value||!0}
else{values[name]=!1}}
...
if($field.filter('#'+identifier).length>0){return!0}
else if($field.filter('[name="'+identifier+'"]').length>0){return!0}
else if($field.filter('[data-'+metadata.validate+'="'+identifier+'"]').length>0){return!0}
...
else if(isCheckbox){module.verbose('Setting checkbox value',value,$element);if(value===!0){$element.checkbox('check')}
...
formErrors=[];if(module.determine.isValid()){module.debug('Form has no validation errors, submitting');module.set.success();if(ignoreCallbacks!==!0){return settings.onSuccess.call(element,event,values)}}
...
if($field.prop('disabled')){module.debug('Field is disabled. Skipping',identifier);fieldValid=!0}
else if(field.optional&&module.is.blank($field)){module.debug('Field is optional and blank. Skipping',identifier);fieldValid=!0}
else if(field.depends&&module.is.empty($dependsField)){module.debug('Field depends on another value that is not present or empty. Skipping',$dependsField);fieldValid=!0}
else if(field.rules!==undefined){$.each(field.rules,function(index,rule){if(module.has.field(identifier)&&!(module.validate.rule(field,rule))){module.debug('Field is invalid',identifier,rule.type);fieldErrors.push(module.get.prompt(rule,field));fieldValid=!1}})}
...
value=(value===undefined||value===''||value===null)?'':$.trim(value+'');return ruleFunction.call($field,value,ancillary)}},setting:function(name,value){if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){settings[name]=value}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
...
else{$activeContent.children().stop(!0,!0).animate({opacity:1},settings.duration,module.resetOpacity)}}
...
else{$activeContent.children().stop(!0,!0).animate({opacity:0},settings.duration,module.resetOpacity)}}
...
else{$openContents.children().stop(!0,!0).animate({opacity:0},settings.duration,module.resetOpacity)}}
...
$dimmer.transition({displayType:'flex',animation:settings.transition+' in',queue:!1,duration:module.get.duration(),useFailSafe:!0,onStart:function(){module.set.dimmed()},onComplete:function(){module.set.active();callback()}})}
...
else{return($module.find(selector.dimmer).length>0)}}},is:{active:function(){return $dimmer.hasClass(className.active)},animating:function(){return($dimmer.is(':animated')||$dimmer.hasClass(className.animating))},closable:function(){if(settings.closable=='auto'){if(settings.on=='hover'){return!1}
return!0}
...
apiSettings={errorDuration:!1,cache:'local',throttle:settings.throttle,urlData:{query:query},onError:function(){module.add.message(message.serverError);callback()},onFailure:function(){module.add.message(message.serverError);callback()},onSuccess:function(response){module.remove.message();module.setup.menu({values:response[fields.remoteValues]});callback()}};if(!$module.api('get request')){module.setup.api()}
...
else if(settings.fullTextSearch==='custom'&&module.customSearch(searchTerm,text)){results.push(this);return!0}
...
return!1},customSearch:function(query,term){query=query.toLowerCase();var input_array=[];input_array=query.match(/\S+/g)||[];term=term.toLowerCase();var found=!0;for(var i=0;i<input_array.length;i++){if(term.indexOf(input_array[i])>-1){}else{found=!1}}
return found},filterActive:function(){if(settings.useLabels){$item.filter('.'+className.active).addClass(className.filtered)}},focusSearch:function(skipHandler){if(module.has.search()&&!module.is.focusedOnSearch()){if(skipHandler){$module.off('focus'+eventNamespace,selector.search);$search.focus();$module.on('focus'+eventNamespace,selector.search,module.event.search.focus)}
else{$search.focus()}}},forceSelection:function(){var
$currentlySelected=$item.not(className.filtered).filter('.'+className.selected).eq(0),$activeItem=$item.not(className.filtered).filter('.'+className.active).eq(0),$selectedItem=($currentlySelected.length>0)?$currentlySelected:$activeItem,hasSelected=($selectedItem.length>0);if(hasSelected&&!module.is.multiple()){module.debug('Forcing partial selection to selected item',$selectedItem);module.event.item.click.call($selectedItem,{},!0);return}
...
else{module.debug('Menu modified, updating selector cache');module.refresh()}},mousedown:function(){itemActivated=!0},mouseup:function(){itemActivated=!1}},item:{mouseenter:function(event){var
...
else{module.verbose('Event occurred in dropdown menu, canceling callback');return!1}}},action:{nothing:function(){},activate:function(text,value,element){value=(value!==undefined)?value:text;if(module.can.activate($(element))){module.set.selected(value,$(element));if(module.is.multiple()&&!module.is.allFiltered()){return}
...
module.debug('Updating input value',escapedValue,currentValue);internalChange=!0;$input.val(escapedValue);if(settings.fireOnInit===!1&&module.is.initialLoad()){module.debug('Input native change event ignored on initial load')}
...
else if($.fn.transition!==undefined&&$module.transition('is supported')){$currentMenu.transition({animation:transition+' out',duration:settings.duration,debug:settings.debug,verbose:settings.verbose,queue:!1,onStart:start,onComplete:function(){callback.call(element)}})}
...
return value.replace(regExp.quote,'&quot;')},string:function(text){text=String(text);return text.replace(regExp.escape,'\\$&')}},setting:function(name,value){module.debug('Changing setting',name,value);if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){if($.isPlainObject(settings[name])){$.extend(!0,settings[name],value)}
else{settings[name]=value}}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
...
return urlString.join('&amp;')}},generate:{embed:function(url){module.debug('Generating embed html');var source=module.get.source(),html,parameters;url=module.get.url(url);if(url){parameters=module.generate.parameters(source);html=templates.iframe(url,parameters)}
...
var $target=$(event.target),isInModal=($target.closest(selector.modal).length>0),isInDOM=$.contains(document.documentElement,event.target);if(!isInModal&&isInDOM&&module.is.active()){module.debug('Dimmer clicked, hiding all modals');module.remove.clickaway();if(settings.allowMultiple){module.hide()}
...
else{module.debug('Modal is already visible')}},hideModal:function(callback,keepDimmed){callback=$.isFunction(callback)?callback:function(){};module.debug('Hiding modal');if(settings.onHide.call(element,$(this))===!1){module.verbose('Hide callback returned false cancelling hide');return}
...
else{module.verbose('Modal cannot fit on screen setting to scrolling');module.set.scrolling()}},undetached:function(){$dimmable.addClass(className.undetached)}},setting:function(name,value){module.debug('Changing setting',name,value);if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){if($.isPlainObject(settings[name])){$.extend(!0,settings[name],value)}
else{settings[name]=value}}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
...
else{module.error(error.noStorage)}}},setting:function(name,value){module.debug('Changing setting',name,value);if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){if($.isPlainObject(settings[name])){$.extend(!0,settings[name],value)}
else{settings[name]=value}}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
...
else if($target.next(selector.popup).length!==0){module.verbose('Pre-existing popup found');settings.inline=!0;settings.popup=$target.next(selector.popup).data(metadata.activator,$module);module.refresh();if(settings.hoverable){module.bind.popup()}}
...
else{return($popup.closest($context).length>=1)?!0:!1}},removePopup:function(){if(module.has.popup()&&!settings.popup){module.debug('Removing popup',$popup);$popup.remove();$popup=undefined;settings.onRemove.call($popup,element)}},save:{conditions:function(){module.cache={title:$module.attr('title')};if(module.cache.title){$module.removeAttr('title')}
...
else{module.error(error.noTransition)}},hide:function(callback){callback=$.isFunction(callback)?callback:function(){};module.debug('Hiding pop-up');if(settings.onHide.call($popup,element)===!1){module.debug('onHide callback returned false, cancelling popup animation');return}
if(settings.transition&&$.fn.transition!==undefined&&$module.transition('is supported')){$popup.transition({animation:settings.transition+' out',queue:!1,duration:settings.duration,debug:settings.debug,verbose:settings.verbose,onComplete:function(){module.reset();callback.call($popup,element);settings.onHidden.call($popup,element)}})}
...
else{module.debug('Popup could not find a position to display',$popup);module.error(error.cannotPlace,element);module.remove.attempts();module.remove.loading();module.reset();settings.onUnplaceable.call($popup,element);return!1}}}
...
offstage=[];$.each(distanceFromBoundary,function(direction,distance){if(distance<-settings.jitter){module.debug('Position exceeds allowable distance from edge',direction,distance,position);offstage.push(direction)}});if(offstage.length>0){return!0}
...
else{module.removePopup()}},setting:function(name,value){if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){settings[name]=value}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
else{return module[name]}},debug:function(){if(!settings.silent&&settings.debug){if(settings.performance){module.performance.log(arguments)}
...
else{percentComplete=value;module.debug('Setting value to exact percentage value',percentComplete);module.set.percent(percentComplete)}}},setting:function(name,value){module.debug('Changing setting',name,value);if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){if($.isPlainObject(settings[name])){$.extend(!0,settings[name],value)}
else{settings[name]=value}}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
...
if(!module.is.initialLoad()){settings.onRate.call(element,rating)}},initialLoad:function(){initialLoad=!0}},setting:function(name,value){module.debug('Changing setting',name,value);if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){if($.isPlainObject(settings[name])){$.extend(!0,settings[name],value)}
else{settings[name]=value}}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
...
disabledBubbled=!1;if(!module.is.animating()&&!module.is.hidden()){callback()}})}
else{module.debug('Input blurred without user action, closing results');callback()}},result:{mousedown:function(){module.resultsClicked=!0},mouseup:function(){module.resultsClicked=!1},click:function(event){module.debug('Search result selected');var $result=$(this),$title=$result.find(selector.title).eq(0),$link=$result.is('a[href]')?$result:$result.find('a[href]').eq(0),href=$link.attr('href')||!1,target=$link.attr('target')||!1,title=$title.html(),value=($title.length>0)?$title.text():!1,results=module.get.results(),result=$result.data(metadata.result)||module.get.result(value,results),returnedValue;if($.isFunction(settings.onSelect)){if(settings.onSelect.call(element,result,results)===!1){module.debug('Custom onSelect callback cancelled default select action');disabledBubbled=!0;return}}
...
if(module.is.visible()){if(keyCode==keys.enter){module.verbose('Enter key pressed, selecting active result');if($result.filter('.'+className.active).length>0){module.event.result.click.call($result.filter('.'+className.active),event);event.preventDefault();return!1}}
...
else if(settings.fullTextSearch==!0&&module.fuzzySearch(searchTerm,content[field])){addResult(fuzzyResults,content)}}})});$.merge(exactResults,fuzzyResults)
...
if(!module.is.visible()&&module.has.results()){if(module.can.transition()){module.debug('Showing results with css animations');$results.transition({animation:settings.transition+' in',debug:settings.debug,verbose:settings.verbose,duration:settings.duration,onComplete:function(){callback()},queue:!0})}
else{module.debug('Showing results with javascript');$results.stop().fadeIn(settings.duration,settings.easing)}
settings.onResultsOpen.call($results)}},hideResults:function(callback){callback=$.isFunction(callback)?callback:function(){};if(module.is.visible()){if(module.can.transition()){module.debug('Hiding results with css animations');$results.transition({animation:settings.transition+' out',debug:settings.debug,verbose:settings.verbose,duration:settings.duration,onComplete:function(){callback()},queue:!0})}
...
else{module.error(error.noTemplate,!1)}}
...
if(!module.is.animating()){module.debug('Flipping up',$nextSide);var transform=module.get.transform.up();module.set.stageSize();module.stage.above();module.animate(transform)}
...
if(!module.is.animating()){module.debug('Flipping down',$nextSide);var transform=module.get.transform.down();module.set.stageSize();module.stage.below();module.animate(transform)}
...
if(!module.is.animating()){module.debug('Flipping left',$nextSide);var transform=module.get.transform.left();module.set.stageSize();module.stage.left();module.animate(transform)}
...
if(!module.is.animating()){module.debug('Flipping right',$nextSide);var transform=module.get.transform.right();module.set.stageSize();module.stage.right();module.animate(transform)}
...
userAgent=navigator.userAgent,isIOS=userAgent.match(regExp.ios),isMobileChrome=userAgent.match(regExp.mobileChrome);if(isIOS&&!isMobileChrome){module.verbose('Browser was found to be iOS',userAgent);return!0}
else{return!1}},mobile:function(){var
...
return module.cache.rtl}},setting:function(name,value){module.debug('Changing setting',name,value);if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){if($.isPlainObject(settings[name])){$.extend(!0,settings[name],value)}
else{settings[name]=value}}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
...
settings=$.extend(!0,{},$.fn.tab.settings,parameters)}}},initializeHistory:function(){module.debug('Initializing page state');if($.address===undefined){module.error(error.state);return!1}
else{if(settings.historyType=='state'){module.debug('Using HTML5 to manage state');if(settings.path!==!1){$.address.history(!0).state(settings.path)}
else{module.error(error.path);return!1}}
...
else{module.debug('Ignoring remote content on first tab load',currentPath);firstLoad=!1;module.cache.add(tabPath,$tab.html());module.activate.all(currentPath);settings.onFirstLoad.call($tab[0],currentPath,parameterArray,historyEvent);settings.onLoad.call($tab[0],currentPath,parameterArray,historyEvent)}
return!1}
else{module.debug('Opened local tab',currentPath);module.activate.all(currentPath);if(!module.cache.read(currentPath)){module.cache.add(currentPath,!0);module.debug('First time tab loaded calling tab init');settings.onFirstLoad.call($tab[0],currentPath,parameterArray,historyEvent)}
...
settings.onLoad.call($tab[0],currentPath,parameterArray,historyEvent);return!1}}
else{module.error(error.missingTab,$module,$context,currentPath);return!1}})},scrollTo:function($element){var
...
$tab=module.get.tabElement(tabPath),tab=$tab[0];evaluateScripts=(evaluateScripts!==undefined)?evaluateScripts:settings.evaluateScripts;if(typeof settings.cacheType=='string'&&settings.cacheType.toLowerCase()=='dom'&&typeof html!=='string'){$tab.empty().append($(html).clone(!0))}
...
settings.onFirstLoad.call($tab[0],tabPath,parameterArray,historyEvent);settings.onLoad.call($tab[0],tabPath,parameterArray,historyEvent);if(settings.loadOnce){module.cache.add(fullTabPath,!0)}
...
$clone=$tab.children().clone(!0);$clone=$clone.not('script');module.cache.add(fullTabPath,$clone)},0)}
else{module.cache.add(fullTabPath,$tab.html())}},urlData:{tab:fullTabPath}},request=$tab.api('get request')||!1,existingRequest=(request&&request.state()==='pending'),requestSettings,cachedContent;fullTabPath=fullTabPath||tabPath;cachedContent=module.cache.read(fullTabPath);if(settings.cache&&cachedContent){module.activate.tab(tabPath);module.debug('Adding cached content',fullTabPath);if(!settings.loadOnce){if(settings.evaluateScripts=='once'){module.update.content(tabPath,cachedContent,!1)}
...
else if($.api!==undefined){requestSettings=$.extend(!0,{},settings.apiSettings,apiSettings);module.debug('Retrieving remote content',fullTabPath,requestSettings);module.set.loading(tabPath);$tab.api(requestSettings)}
...
return typeof pathName=='string'?pathName.split('/'):[pathName]},arrayToPath:function(pathArray){return $.isArray(pathArray)?pathArray.join('/'):!1}},setting:function(name,value){module.debug('Changing setting',name,value);if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){if($.isPlainObject(settings[name])){$.extend(!0,settings[name],value)}
else{settings[name]=value}}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
...
$clone.remove();if(currentAnimation!=inAnimation){module.debug('Direction exists for animation',animation);directionExists=!0}
...
else{module.verbose('XHR request was not aborted');return!1}},validResponse:function(response){if((!module.is.expectingJSON())||!$.isFunction(settings.successTest)){module.verbose('Response is not JSON, skipping validation',settings.successTest,response);return!0}
...
canSerialize=($.fn.serializeObject!==undefined),formData=(canSerialize)?$form.serializeObject():$form.serialize(),hasOtherData;data=data||settings.data;hasOtherData=$.isPlainObject(data);if(hasOtherData){if(canSerialize){module.debug('Extending existing data with form data',data,formData);data=$.extend(!0,{},data,formData)}
...
textStatus=!1,status=!1,httpMessage=!1,responder=settings.mockResponse||settings.response,asyncResponder=settings.mockResponseAsync||settings.responseAsync,asyncCallback,response,mockedXHR;mockedXHR=$.Deferred().always(module.event.xhr.complete).done(module.event.xhr.done).fail(module.event.xhr.fail);if(responder){if($.isFunction(responder)){module.debug('Using specified synchronous callback',responder);response=responder.call(context,requestSettings)}
...
else{return settings.on}},templatedURL:function(action){action=action||$module.data(metadata.action)||settings.action||!1;url=$module.data(metadata.url)||settings.url||!1;if(url){module.debug('Using specified url',url);return url}
...
else if(module.is.form()){url=$module.attr('action')||$context.attr('action')||!1;module.debug('No url or action specified, defaulting to form action',url)}
return url}},abort:function(){var
xhr=module.get.xhr();if(xhr&&xhr.state()!=='resolved'){module.debug('Cancelling API request');xhr.abort()}},reset:function(){module.remove.error();module.remove.loading()},setting:function(name,value){module.debug('Changing setting',name,value);if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){if($.isPlainObject(settings[name])){$.extend(!0,settings[name],value)}
else{settings[name]=value}}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
...
$.when(apiRequest).then(function(){if(apiRequest.state()=='resolved'){module.debug('API request succeeded');settings.activateTest=function(){return!0};settings.deactivateTest=function(){return!0}}
else{module.debug('API request failed');settings.activateTest=function(){return!1};settings.deactivateTest=function(){return!1}}
...
else{$allModules.not($module).state('deactivate')}},get:{text:function(){return(settings.selector.text)?$module.find(settings.selector.text).text():$module.html()},textFor:function(state){return text[state]||!1}},flash:{text:function(text,duration,callback){var
...
else{module.debug('Text is already set, ignoring update',text)}}},setting:function(name,value){module.debug('Changing setting',name,value);if($.isPlainObject(name)){$.extend(!0,settings,name)}
else if(value!==undefined){if($.isPlainObject(settings[name])){$.extend(!0,settings[name],value)}
else{settings[name]=value}}
else{return settings[name]}},internal:function(name,value){if($.isPlainObject(name)){$.extend(!0,module,name)}
else if(value!==undefined){module[name]=value}
